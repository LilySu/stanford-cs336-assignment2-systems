...
class RotaryEmbedding(nn.Module):
    ...
    # REMOVED: @torch._dynamo.disable (We want this to be compiled!)
    def forward(self, x: Float[Tensor, " ... seq d"], pos_ids: Int[Tensor, " ... seq"]) -> Float[Tensor, " ... seq d"]:
        x1, x2 = rearrange(x, '... (half_d xy) -> xy ... half_d', xy=2)

        # Use Standard PyTorch Indexing ---
        # self._freq_cis_cache shape: [2, max_len, half_dim]
        # pos_ids shape: [batch, seq_len]
        # Result shape: [2, batch, seq_len, half_dim]
        freqs = self._freq_cis_cache[:, pos_ids, :] 
        ...
        # Use rearrange from einops instead of einx ---
        result = rearrange([x1_rot, x2_rot], 'xy ... half_d -> ... (half_d xy)', xy=2)
        return result
